...
func (r *hotelDayViewRepo) ListSchedule(ctx context.Context, start, end time.Time) ([]model.HotelDayViewTherapist, error) {
	ctx, cancel := db.WithQueryTimeout(ctx)
	defer cancel()
	// This hotel-facing schedule intentionally selects occupancy only. Therapist
	// identities and booking details never enter the response path.
	...
	for rows.Next() {
		...
		if err := rows.Scan(&therapist.TherapistID, &therapist.BranchID, &therapist.AcceptingBookings, &slotStart, &slotEnd); err != nil {
			return nil, err
		}
