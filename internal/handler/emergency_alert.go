...
func (h *EmergencyAlertHandler) TriggerAlert(w http.ResponseWriter, r *http.Request) {
	...
	if h.bookingService != nil {
		if b, _, svc, addr, _, _, berr := h.bookingService.GetBookingWithTimeline(r.Context(), alert.BookingID, alert.TriggeredBy); berr == nil && b != nil {
			cName, cPhone, cPhoto := h.bookingService.FetchClientInfo(r.Context(), b.ClientID)
			br := toBookingResponse(b, svc, addr, "", nil, cName, cPhone, cPhoto)
			bookingResp = &br
		}
	}
