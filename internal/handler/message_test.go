...
func TestCreateConversation_InvalidBody_ReturnsStructuredError(t *testing.T) {
	h := NewMessageHandler(nil)

	rr := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/conversations", bytes.NewBufferString("not-json"))

	h.CreateConversation(rr, req)
